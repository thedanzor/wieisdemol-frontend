  ...
  return <ListWrapper>
    <div>
      ...
      <div className='mainscreen-container'>
        {
          bets && bets.length && bets.map((betweek, index) => {
            return <div className='bet-history-container'><SectionTitle> Week {betweek.week} </SectionTitle>
              {
                processing
                  ? <div className='loader-wrapper'> <Loader /> </div>
                  : bets && bets.length > 0
                    ? <BetContainer players={players} betWeek={betweek} />
                    : 'No data found'
              }
            </div>
          })
